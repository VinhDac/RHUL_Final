"""test_mlp.py - sanity checks for the MLP, PRINT style (exits 1 on any FAIL).

Run from the repo root:   python -m tests.test_mlp

Trains the MLP on Case 2 (learnable) and Case 1 (random labels) and prints
train / test accuracy. Expected:
  Case 2 -> test ~1.0  (it learns sign(feature 0))
  Case 1 -> test ~0.5  (no signal to generalise) while train > 0.5 (fits some noise)
"""
import sys
import numpy as np
...
print("  -> expect test ~1.0  ->", result(te_a > 0.9 and te_b > 0.9))
...
print(f"  width 16  : train {tr_a:.3f}   test {te_a:.3f}")
print(f"  width 128 : train {tr_b:.3f}   test {te_b:.3f}")
print("  -> expect test ~0.5 (no generalisation) ->", result(0.45 < te_a < 0.55 and 0.45 < te_b < 0.55))
print("  -> train > 0.5 = fits some of the noise (see §6.2) ->", result(tr_a > 0.5 and tr_b > 0.5))

print("\n=== CONFIG SAMPLING (the random search space, §3) ===")
...
print("  -> widths in {4..256}?", result(ok_w), "| lr in [0.01, 1.0]?", result(ok_lr))
